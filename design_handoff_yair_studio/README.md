...
> ⚠️ **Legacy snapshot.** This folder is not the current source of truth. For current brand rules, use the root `README.md`, `assets/`, `preview/`, and `ui_kits/`.
...
The brand idea is the wordmark itself: **y[AI]r** — the specialism is baked into the name. The **visual logo** sets `AI` inside an electric orange knockout block, flanked by a lowercase `y` and `r` with subtle hairline dashes. The **written brand** stays `y[AI]r studio` — square brackets around uppercase `AI` — in all body copy, docs, and accessible labels. The `[AI]` block alone is the avatar / favicon mark.
